...
const avatar = process.env.PUBLIC_URL + '/avatar1.png';
const customStyles = {
    content: {
        top: '50%',
        left: '50%',
...
const RecordHeader = ({number, item, quantity, price, buyer, seller, time}) => {
    return (<div className={'record'}>
        <div>{number}</div>
        <div>{item}</div>
        <div>{quantity}</div>
